...
To run
-------
python tutorial_***.py --train/test_transformer
...
def D():  # some common functions, could be extracted into exercise afterwards
    ''' description of function '''
...
if __name__ == '__main__':
    ...
    if args.train:
        ...
        ''' plot , following the format of ./baselines/exercise/plot()'''
        plot(rewards, Algorithm_name='SAC', Env_name='Pendulum-v0')
        ''' save weights, implemented in defined classes above, following the format of ./baselines/exercise/save_model()  '''
        model.save_weights()

    # testing loop
    if args.test:
        t0 = time.time()
        ''' save weights, implemented in defined classes above, following the format of ./baselines/exercise/load_model()  '''
        model.load_weights()
